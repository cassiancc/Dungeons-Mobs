...
public class ArmorGearRenderer<T extends ArmorGear & GeoItem> extends GeoArmorRenderer<T> {
    ...
    public void prepMatrixForBone(PoseStack stack, GeoBone bone) {
        ...
        if (this.currentStack.getItem() instanceof ArmorGear armorGear) {
            if (bone.getName().contains("Body") && armorGear.getDungeonsArmorMaterial().getBaseType() == ArmorMaterialBaseType.CLOTH) {
